 ...
 *   A utfordrer B → B aksepterer/avslår → duell-aktivitet kjøres → vinner registreres.
 * Aktivitetene har to former:
 *   - 'manuell': læreren/gruppene dømmer selv (eksisterende Vi vant/De vant-knapper).
 *   - 'tapping' | 'reaksjon' | 'regning': avgjøres i appen. Hver forkjemper kjører
 *     sitt mini-spill på egen enhet, resultatet skrives til Firebase, og vinneren
 *     beregnes deterministisk når begge er ferdige.
 * Utfall: vinner +3 handel, taper −2 handel, begge −1 rykte. Hver gruppe anvender
 * sitt eget utfall én gang (de-dupes via localStorage).
 */
...
/** Deterministisk vinnerberegning fra to forkjempere-resultater. Tie = utfordrer. */
function computeWinner(
  kind: 'tapping' | 'reaksjon' | 'regning',
  challengerId: string, rA: DuelChampionResult,
  defenderId: string, rB: DuelChampionResult,
): string {
  if (kind === 'reaksjon') {
    const a = rA.reactionMs ?? Infinity;
    const b = rB.reactionMs ?? Infinity;
    return a <= b ? challengerId : defenderId;
  }
  // tapping, regning: høyere score vinner
  return (rA.score ?? 0) >= (rB.score ?? 0) ? challengerId : defenderId;
}
...
export default function SeaBattle({ code, myGroupId, myShipName, mySkills, onResult }: Props) {
  ...
  const submittedResolution = useRef<Set<string>>(new Set());
  ...
  // Beregn vinneren for in-app dueller når begge forkjempere har sendt resultat.
  // Begge klienter kan skrive — det er deterministisk, så samme winnerId.
  useEffect(() => {
    for (const [id, d] of Object.entries(duels)) {
      if (d.status !== 'active') continue;
      const kind = d.activity.kind;
      if (!kind || kind === 'manuell') continue;
      if (d.challengerId !== myGroupId && d.defenderId !== myGroupId) continue;
      if (submittedResolution.current.has(id)) continue;
      const rA = d.championResults?.[d.challengerId];
      const rB = d.championResults?.[d.defenderId];
      if (!rA || !rB) continue;
      submittedResolution.current.add(id);
      const winnerId = computeWinner(kind, d.challengerId, rA, d.defenderId, rB);
      updateDuel(code, id, { status: 'resolved', winnerId }).catch(() => {});
    }
  }, [duels, myGroupId, code]);
  ...
  return (
    <MaterialPanel material="jern" className="border-2 border-viking-crimson/50 p-5">
      ...
      {active.map(([id, d]) => {
        ...
        const kind = d.activity.kind;
        const myResult = d.championResults?.[myGroupId];
        const otherResult = d.championResults?.[otherId];
        const isManuell = !kind || kind === 'manuell';

        return (
          <div key={id} className="mb-3 rounded-lg border-2 border-viking-gold bg-viking-darkblue/60 p-4" data-testid="active-duel">
            ...
            <p className="mb-3 font-inter text-sm text-viking-paper/85">{d.activity.desc}</p>

            {isManuell ? (
              <>
                <p className="mb-3 font-mono text-xs text-viking-gold-soft">Deres fordel i {skillTreeData[d.activity.ferdighet].name}: +{mySkills[d.activity.ferdighet] ?? 0}</p>
