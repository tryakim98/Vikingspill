...
export function Yggdrasil({ size = 180, className = '', color = '#D4A843' }: DecorProps) {
  return (
...
// ─── Mjølner — Tors hammer ──────────────────────────────────────────────────
export function ThorHammer({ size = 48, className = '', color = '#D4A843' }: DecorProps) {
  return (
...
export function Raven({ size = 56, className = '', color = '#17100A', facing = 'right' }: DecorProps & { facing?: 'left' | 'right' }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" className={className} aria-hidden="true">
      <g transform={flip} fill={color} stroke="#D4A843" strokeWidth="1.2" strokeLinejoin="round">
        {/* Kropp */}
        ...
        {/* Nebb */}
        <path d="M 54 22 L 62 21 L 54 24 Z" fill="#D4A843" />
        {/* Øye */}
...
export function Vegvisir({ size = 80, className = '', color = '#D4A843' }: DecorProps) {
...
export function RuneDivider({ className = '', color = '#D4A843', runes = ['ᚦ', 'ᚱ', 'ᚾ', 'ᛏ', 'ᛚ', 'ᛟ'] }: { className?: string; color?: string; runes?: string[] }) {
  return (
...
export function DragonHead({ size = 72, className = '', color = '#D4A843', facing = 'right' }: DecorProps & { facing?: 'left' | 'right' }) {
