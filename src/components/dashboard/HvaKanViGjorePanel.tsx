...
export default function HvaKanViGjorePanel({ destinations, scores, skills, goods, visited, locked, unlockedSides }: Props) {
  ...
  return (
    <div className="mb-6 rounded-lg border-2 border-viking-gold/50 bg-viking-darkblue/40 p-4" data-testid="hva-kan-vi-gjore">
      <h3 className="mb-2 font-cinzel text-base text-viking-gold">🧭 Hva kan vi gjøre?</h3>
      ...
      <div className="mb-3 grid grid-cols-2 gap-1.5 text-[11px] sm:grid-cols-3" data-testid="resource-key">
        <ResourceTip icon="📖" label="Kulturforståelse" tip="Bedre valg, åpner sidesteder" />
        <ResourceTip icon="💰" label="Handel" tip="Kjøp varer, betal spesielle handlinger" />
        <ResourceTip icon="🏆" label="Rykte" tip="Diplomati-valg, fjerne havner" />
        <ResourceTip icon="📦" label="Handelsvarer" tip="Lås opp sidesteder, bytt med andre" />
        <ResourceTip icon="⚔" label="Ferdigheter" tip="Bonus på terning, låser opp valg" />
        <ResourceTip icon="📜" label="Svenneprøver" tip="Lås opp sidesteder via quiz" />
      </div>

      {/* Klare til å seile */}
      {openNow.length > 0 && (
        <div className="mb-2 rounded-md border border-viking-moss/50 bg-viking-moss/10 px-3 py-2" data-testid="open-now-section">
          <p className="font-cinzel text-xs font-bold text-viking-moss">⛵ Klare til å seile dit nå</p>
      ...
      {withinReach.length > 0 && (
        <div className="mb-2 rounded-md border border-viking-gold/40 bg-viking-gold/5 px-3 py-2" data-testid="within-reach-section">
          <p className="font-cinzel text-xs font-bold text-viking-gold">🎯 Innen rekkevidde</p>
          <ul className="mt-1 space-y-0.5">
            {withinReach.map((r) => (
      ...
      {far.length > 0 && (
        <div className="rounded-md border border-viking-gold/20 bg-viking-surface/30 px-3 py-2" data-testid="far-section">
          <p className="font-cinzel text-xs text-viking-gold-soft/80">🌫 Lengre fram</p>
          <ul className="mt-1 space-y-0.5">
            {far.map((r) => (
...
function ResourceTip({ icon, label, tip }: { icon: string; label: string; tip: string }) {
  return (
    <div className="flex items-start gap-1.5 rounded-md border border-viking-gold/25 bg-viking-surface/40 px-2 py-1">
