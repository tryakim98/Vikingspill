...
export default function TradeGoodsPanel({ goods }: Props) {
  ...
  return (
    <MaterialPanel material="tre" framed className="p-3" data-testid="trade-goods-panel">
      ...
      {total === 0 ? (
        <p className="font-inter text-xs italic text-viking-paper/70">Lasterommet er tomt — fullfør destinasjoner for å samle varer.</p>
      ) : (
          ...
          {ORDER.map((g) => {
            ...
            return (
              <div
                ...
                title={info.origin}
                data-testid={`good-${g}`}
                className="flex items-center gap-1.5 border border-viking-gold/40 bg-black/35 px-2 py-1"
              >
                ...
                <span className="font-inter text-xs text-viking-paper">{info.name}</span>
